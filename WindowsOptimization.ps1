...
Begin {
        ...
        try {
            ...
            # Local configuration root near this script. If ./ConfigurationFiles exists, use it; otherwise use the script folder.
            if (Test-Path (Join-Path $PSScriptRoot 'ConfigurationFiles')) {
                $ConfigRoot = Join-Path $PSScriptRoot 'ConfigurationFiles'
            } else {
            ...
            # JSON configuration files are expected to be located under: $ConfigRoot
...
PROCESS {
    ...
    If ($Optimizations -contains 'ScheduledTasks' -or $Optimizations -contains "All") {
        ...
        try 
        {
            $ScheduledTasksFilePath = Join-Path -Path $ConfigRoot -ChildPath 'ScheduledTasks.json'
            ...
            If (Test-Path $ScheduledTasksFilePath)
            {
                ...
                $SchTasksList = (Get-Content $ScheduledTasksFilePath | ConvertFrom-Json) | Where-Object { $_.VDIState -eq 'Disabled' }
    ...
    {
    $DefaultUserSettingsFilePath = Join-Path -Path $ConfigRoot -ChildPath 'DefaultUserSettings.json'

        If (Test-Path $DefaultUserSettingsFilePath)
        {
            Write-Host "AVD AIB Customization : Windows Optimizations - - Set Default User Settings"
            $UserSettings = (Get-Content $DefaultUserSettingsFilePath | ConvertFrom-Json) | Where-Object { $_.SetProperty -eq $true }
    ...
    {
    $AutoLoggersFilePath = Join-Path -Path $ConfigRoot -ChildPath 'Autologgers.json'
      
        If (Test-Path $AutoLoggersFilePath)
        {
            Write-Host "AVD AIB Customization : Windows Optimizations- Disable AutoLoggers"

            $DisableAutologgers = (Get-Content $AutoLoggersFilePath | ConvertFrom-Json) | Where-Object { $_.Disabled -eq 'True' }
    ...
    $ServicesFilePath = Join-Path -Path $ConfigRoot -ChildPath 'Services.json'

        If (Test-Path $ServicesFilePath)
        {
            Write-Host "AVD AIB Customization : Windows Optimizations- Disable Services" 
            $ServicesToDisable = (Get-Content $ServicesFilePath | ConvertFrom-Json) | Where-Object { $_.VDIState -eq 'Disabled' }
    ...
    {
    $NetworkOptimizationsFilePath = Join-Path -Path $ConfigRoot -ChildPath 'LanManWorkstation.json'
    ...
    {
    $LocalPolicyFilePath = Join-Path -Path $ConfigRoot -ChildPath 'PolicyRegSettings.json'
    ...
    {
    $EdgeFilePath = Join-Path -Path $ConfigRoot -ChildPath 'EdgeSettings.json'
            ...
            If ($EdgeSettings.Count -gt 0)
            {
                ...
                Foreach ($Key in $EdgeSettings)
                {
                    If ($Key.VDIState -eq 'Enabled')
                    {
                        ...
                        {
                            $defaultAssoc = Join-Path $ConfigRoot 'DefaultAssociationsConfiguration.xml'
                            Copy-Item $defaultAssoc $key.RegItemValue -Force
                        }
                        If (Get-ItemProperty -Path $Key.RegItemPath -Name $Key.RegItemValueName -ErrorAction SilentlyContinue) 
                        { 
                            Write-Host "AVD AIB Customization : Windows Optimizations - Found key, $($Key.RegItemPath) Name $($Key.RegItemValueName) Value $($Key.RegItemValue)"
